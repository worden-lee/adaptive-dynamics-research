...
class TrajectoryInterruptedException(DynamicsException):
    # Exception meaning the integration of the dynamics
    # has been interrupted. provides the partial trajectory.
    # subclasses below are the ones that should be used, to
    # explain what was the cause.
    def __init__(self, trajectory, text):
	Exception.__init__(self, text)
	self._trajectory = trajectory

class EquilibriumDetectedException(TrajectoryInterruptedException):
    def __init__(self, trajectory=None):
        TrajectoryInterruptedException.__init__( self, trajectory, 'Equilibrium detected' )

class UnboundedDynamicsException(TrajectoryInterruptedException):
    def __init__(self, trajectory, message=None):
        if message is None:
            message = 'Dynamics has become unbounded'
        TrajectoryInterruptedException.__init__( self, trajectory, message )
...
class CompositeTrajectory( Trajectory ):
	...
	self._series = trajs
    def __repr__(self):
	return 'CompositeTrajectory: ' + ', '.join( t.__repr__() for t in self._series )
    def append(self, state):
	self._series[-1].append(state)
    def make_points(self, xexpr, yexpr):
	return sum( (t.make_points( xexpr, yexpr ) for t in self._series), [] )
    def __add__(self, other):
	try:
		return CompositeTrajectory( *(self._series + other._series) )
	except AttributeError: # other isn't composite
		return CompositeTrajectory( *(self._series + [other]) )
    def write_csv(self, filename, *columns):
	raise ValueError, 'Haven\'t worked out how to write composite trajectory to csv'
...
class Bindings(dict):
    ...
    def __init__(self, *args, **named_args):
        ...
        #print ' =>', self
    def __repr__(self):
        try: frepr = repr(self._function_bindings)
	except AttributeError: frepr = ''
        if frepr != '':
            frepr = ', ' + frepr
	...
	## if it has a bind method, that'll work
	try:
...
class FunctionBindings(Bindings):
    ...
    def __init__(self, *args, **named_args):
	    ...
	    for k,v in a.items():
			...
			print (k,v), 'as expr:function?'
                	#self.update( { (str(k),SR(v).arguments()):SR(v) } )
			#print { (str(k),SR(v).arguments()):SR(v) }
                	self.update( { (str(k),v.arguments()):v } )
			print { (str(k),v.arguments()):v }
		    except ValueError:
...
class ODESystem(SageObject):
    ...
    def gsl_system(self):
	n = len(self._vars)
	if not n in self._gsl_module_cache:
            ...
            print cython_code
            self._gsl_module_cache[n] = sage.misc.cython.compile_and_load( cython_code )
        T = ode_solver()
        #T.algorithm = 'bsimp'
        T.function = self._gsl_module_cache[n].gsl_ode_system( *(
...
# unlike the standard odeint(), this stops when the du/dt function
# raises an exception
...
class PopulationDynamicsSystem(ODESystem):
	...
	try: index_change = (xi != self._population_indices)
	except AttributeError: index_change = True
	if index_change:
            self._population_indices = xi
	    try: del(self._gsl_system)
	    except AttributeError: pass

...
ltx = latex_output( 'sea-ass.sage.out.tex' )

sea_ass = sea_f.bind( base_b, exp_ass_b, fixed_tran_b )

ltx.write( 'To study the incentive structure for association, we set', base_b, exp_ass_b, fixed_tran_b )
...
ltx.write( 
	dgroup( [ [ dot(v), latex_partials_representation()( sea_adap_ass_formal._flow[v] ) ] for v in sea_adap_ass_formal._vars ], op='=' ),
	'Or ', sea_adap_ass )
...
ltx = latex_output( 'sea-ass-sel.tex' )

ltx.write( 'The selective pressures on investment are' )
ltx.write( dgroup( [ [ a_indexer[i], latex_partials_representation()( sea_adap_ass_formal._S[a_indexer[i]] ) ] for i in sea_adap_ass._popdyn_model._population_indices ], op=r'\to' ) )

ltx.write( 'So the condition for increase in guest investment is' )
ltx.write_block( latex_partials_representation()( sea_adap_ass_formal._S[a_indexer[(seamodel.type_g,0)]] ) > 0 )

ltx.write( 'And for increase in host investment' )
ltx.write_block( latex_partials_representation()( sea_adap_ass_formal._S[a_indexer[(seamodel.type_H,0)]] ) > 0 )

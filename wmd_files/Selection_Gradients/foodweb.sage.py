...
_sage_const_1 = Integer(1); _sage_const_0 = Integer(0)# requires: $(SageDynamics)/dynamicalsystems.py
...
class FoodWebModel(dynamicalsystems.PopulationDynamicsSystem):
	...
	x_indices = [ (v,_sage_const_0 ) for v in graph.vertices() ]
    ...
    def flow(self):
	inflo = { v: _sage_const_0  for v in self._vars }
	outflo = { v: _sage_const_0  for v in self._vars }
	k = self._bindings( 'k' )
	for i in self._population_indices:
	    xi = self._population_indexer[i]
	    for j in self._population_indices:
		if i != j and self._graph.has_edge( j[_sage_const_0 ], i[_sage_const_0 ] ):
		    xj = self._population_indexer[j]
		    axx = self._a( i, j ) * xi * xj
		    outflo[ xj ] += axx
		    inflo[ xi ] += k * axx
	    if inflo[xi] == _sage_const_0 :
	...
	i = _sage_const_1  + max( i[_sage_const_1 ] for i in self._population_indices )
	return (index[_sage_const_0 ], i)
